...
def _dump_sus(
    ...
    for measure, length in sorted_bl:
        lines.append(f"#{measure:03d}02: {_format_number(length)}")
    lines.append("")
    ...
    for m in sorted(bpm_by_measure.keys()):
        ...
        lines.append(f"#{m:03d}08: {''.join(bpm_data)}")
    lines.append("")
    ...
    def write_note_lines() -> list[str]:
        ...
        for measure in sorted(measures_map.keys()):
            ...
            for info, nm in note_map.items():
                ...
                result.append(f"#{measure:03d}{info}:{sep.join(data)}")
                ...
                while conflicts:
                    ...
                    result.append(f"#{measure:03d}{info}:{sep.join(data2)}")
                    conflicts = temp
