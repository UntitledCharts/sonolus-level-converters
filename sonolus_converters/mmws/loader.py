...
def read_metadata(fbin: BinaryIO):
    return MetaData(
        ...
        artist = (read_cstr(fbin, 64), read_cstr(fbin, 64))[0], # artist + musicFile(read to advance buffer)
        waveoffset = read_float(fbin) / -1000,
        requests=[]
    )

def read_events(fbin: BinaryIO, version: Version, time_scale_groups: list[TimeScaleGroup]):
    ...
    for _ in range(tempo_count):
        tempo = Bpm(
            beat=tick_to_beat(read_int(fbin)),
            bpm=read_float(fbin)
        )
        events.append(tempo)
...
TYPE_PARAMS: dict[Type, Iterable[str]] = {
    Single: ('beat', 'critical', 'lane', 'size', 'fake', 'timeScaleGroup', 'trace', 'direction'),
    GuidePoint: ('beat', 'ease', 'lane', 'size', 'timeScaleGroup'),
    SlideStartPoint: ('beat','critical','ease','judgeType','lane','size','timeScaleGroup'),
    SlideRelayPoint: ('beat','ease','lane','size','timeScaleGroup','critical','type'),
    SlideEndPoint: ('beat','critical','judgeType','lane','size','timeScaleGroup','direction')
}
def data_init[T](data_type: Type[T], data: dict):
    return data_type(**{k: data[k] for k in TYPE_PARAMS[data_type] if k in data})

def read_note_data(fbin: BinaryIO, version: Version, type: Literal['tap', 'start', 'mid', 'end']):
...
def read_holds(fbin: BinaryIO, version: Version):
    ...
    for _ in range(hold_count):
        ...
        if is_guide:
            ...
            hold_start = data_init(SlideStartPoint, {'judgeType': start_judge, **start_data})

        if version.has_fadeType:
            fade_type = read_int(fbin)
        guide_color = read_int(fbin) if version.has_guideColor else (GuideColor.YELLOW if start_data['critical'] else GuideColor.GREEN)

        hold_step_count = read_int(fbin)
        if is_guide:
            guide = Guide(color=guide_color_to_color(guide_color), fade=fade_type_to_fade(fade_type), midpoints=[guide_start])
            for _ in range(hold_step_count):
                guide.append(data_init(GuidePoint, read_note_data(fbin, version, 'mid')))
            guide.append(data_init(GuidePoint, {'ease': 'linear', **read_note_data(fbin, version, 'end')}))
            holds.append(guide)
        else:
            slide = Slide(critical=start_data['critical'], fake=is_fake, connections=[hold_start])
            for _ in range(hold_step_count):
                slide.append(data_init(SlideRelayPoint, read_note_data(fbin, version, 'mid')))
            slide.append(data_init(SlideEndPoint, {'judgeType': end_judge, **read_note_data(fbin, version, 'end')}))
            holds.append(slide)
    return holds
...
def load(fp: TextIO) -> Score:
    ...
    match signature:
        ...
        case Signature.MikuMikuWorld4ChartCyanvas:
            version = Version(0, version=read_int(fbin, 16), cc_version= read_int(fbin, 16))
    ...
    notes_data: list[Bpm | TimeScaleGroup | Single | Skill | FeverStart | FeverChance | Slide | Guide] = []
