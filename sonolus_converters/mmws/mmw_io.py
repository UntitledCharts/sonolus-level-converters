...
from typing import BinaryIO, Literal, Callable, TypeVar
...
def read_float(f: BinaryIO, bit_size: Literal[32, 64] = 32):
    return float(
        struct.unpack("<f" if bit_size == 32 else "<d", f.read(bit_size // 8))[0]
    )
...
def write_int(
    f: BinaryIO, value: int, bit_size: Literal[8, 16, 32, 64] = 32, signed=False
):
...
class Version:
    ...
    def __init__(
        self,
        uc_version=MikuMikuWorld4UntitledChart,
        cc_version=MikuMikuWorld4ChartCyanvas,
        version=MikuMikuWorld,
    ):
        ...
        if uc_version:
            ...
            self.value = int.from_bytes(
                struct.pack("<HH", version, cc_version), "little"
            )
        else:
            self.value = version
...
def to_mmw_lane(
    usc_lane: float, size: float, result_t: Callable[[float], T] = float
) -> T:
    match result_t(0):
        case int(0):
...
def size_to_width(size: float, result_t: Callable[[float], T] = float) -> T:
...
def flick_to_direction(flick_type: int):
    match flick_type:
        ...
        case FlickType.RIGHT:
            return "right"
        case _:
            raise ValueError(f"Unknown flick type: {flick_type}")
...
def ease_num_to_ease(ease_num: int):
    match ease_num:
        ...
        case EaseType.EASEOUTIN:
            return "outin"
        case _:
            raise ValueError(f"Unknown ease type: {ease_num}")
...
def ease_to_ease_num(ease: Literal["outin", "out", "linear", "in", "inout"]):
    match ease:
        ...
        case _:
            raise ValueError(f"Unknown ease string: {ease}")
...
def guide_color_to_color(guide_color: int):
    match guide_color:
        ...
        case 7:
            return "black"
        case _:
            raise ValueError(f"Unknown guide color index: {guide_color}")
...
def color_to_guide_color(
    ...
    match color:
        ...
        case "black":
            return 7
        case _:
            raise ValueError(f"Unknown guide color: {color}")
...
def fade_type_to_fade(fade_type: int):
    match fade_type:
        ...
        case 2:
            return "in"
        case _:
            raise ValueError(f"Unknown fade type index: {fade_type}")
...
def fade_to_fade_type(fade: Literal["out", "none", "in"]) -> int:
    match fade:
        ...
        case "in":
            return 2
        case _:
            raise ValueError(f"Unknown fade type: {fade}")
...
class NoteGroups:
    ...
    def by(self, object_type: type[T]) -> list[T]:
