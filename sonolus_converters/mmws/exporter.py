...
def write_metadata(
    fbin: BinaryIO, version: Version, metadata: MetaData, notes: list[Any] = []
):
    ...
    if version.has_laneExtension:
        lane_extension = math.ceil(
            max(usc_object_get_max_lane_offset(note) for note in notes)
        )
...
def write_events(fbin: BinaryIO, version: Version, noteGroup: NoteGroups):
    ...
    if version.has_hispeed:
        ...
        time_scale_count = (
            sum(len(group.changes) for group in groups)
            if version.has_layers
            else len(groups[0].changes)
        )
...
@overload
def write_note_data(
    fbin: BinaryIO, version: Version, note: Single, note_type: Literal["tap"]
): ...
@overload
def write_note_data(
    fbin: BinaryIO, version: Version, note: SlideStartPoint, note_type: Literal["start"]
): ...
@overload
def write_note_data(
    fbin: BinaryIO,
    version: Version,
    note: SlideRelayPoint,
    note_type: Literal["mid"],
    *,
    slide: Slide | None = None,
): ...
@overload
def write_note_data(
    fbin: BinaryIO, version: Version, note: SlideEndPoint, note_type: Literal["end"]
): ...
@overload
def write_note_data(
    fbin: BinaryIO,
    version: Version,
    note: GuidePoint,
    note_type: Literal["start", "mid", "end"],
): ...
...
def write_note_data(
    ...
    *,
    ...
    if getattr(note, "critical", False) or (
        note_type == "mid" and slide and slide.critical
    ):
...
def write_holds(fbin: BinaryIO, version: Version, noteGroup: NoteGroups):
    ...
    for guide in guides:
        ...
        for i, point in enumerate(guidePoints):
            write_note_data(
                fbin, version, point, "mid" if i == (len(guidePoints) - 1) else "end"
            )
...
def export(
    ...
    score: Score,
    *,
    format: Literal[".mmws", ".ccmmws", ".unchmmws"] | str | None = None,
):
    ...
    try:
        ...
        if version.has_address:
            address_size = 4 * (
                4 + version.has_damageNote + version.has_layers + version.has_waypoints
            )
